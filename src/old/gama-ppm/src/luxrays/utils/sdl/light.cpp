...
float PerezBase(const float lam[6], float theta, float gamma) {
	return (1.f + lam[1] * expf(lam[2] / cosf(theta))) * (1.f + lam[3] * expf(
			lam[4] * gamma) + lam[5] * cosf(gamma) * cosf(gamma));
}

inline float RiAngleBetween(float thetav, float phiv, float theta, float phi) {
	const float cospsi = sinf(thetav) * sinf(theta) * cosf(phi - phiv) + cosf(
			thetav) * cosf(theta);
	if (cospsi >= 1.f)
		return 0.f;
	if (cospsi <= -1.f)
...
void SkyLight::Init() {
	...
	zenith_x = (0.00166f * theta3 - 0.00375f * theta2 + 0.00209f * thetaS) * T2
			+ (-0.02903f * theta3 + 0.06377f * theta2 - 0.03202f * thetaS
					+ 0.00394f) * T + (0.11693f * theta3 - 0.21196f * theta2
			+ 0.06052f * thetaS + 0.25886f);

	zenith_y = (0.00275f * theta3 - 0.00610f * theta2 + 0.00317f * thetaS) * T2
			+ (-0.04214f * theta3 + 0.08970f * theta2 - 0.04153f * thetaS
					+ 0.00516f) * T + (0.15346f * theta3 - 0.26756f * theta2
			+ 0.06670f * thetaS + 0.26688f);
...
void SkyLight::GetSkySpectralRadiance(const float theta, const float phi,
		Spectrum * const spect) const {
...
SunLight::SunLight(float turb, float size, const Vector &sd) :
	LightSource() {
	turbidity = turb;
	sundir = Normalize(sd);
	gain = Spectrum(1.0f, 1.0f, 1.0f);
...
void SunLight::Init() {
	...
	for (i = 0, lambda = 350.f; i < 91; ++i, lambda += 5.f) {
		...
		tauG = expf(
				-1.41f * k_gCurve.sample(lambda) * m / powf(
						1.f + 118.93f * k_gCurve.sample(lambda) * m, 0.45f));
		// Attenuation due to water vapor absorbtion
		// w - precipitable water vapor in centimeters (standard = 2)
		const float w = 2.0f;
		tauWA
				= expf(
						-0.2385f * k_waCurve.sample(lambda) * w * m
								/ powf(
										1.f + 20.07f * k_waCurve.sample(lambda)
												* w * m, 0.45f));

		Ldata[i]
				= (solCurve.sample(lambda) * tauR * tauA * tauO * tauG * tauWA);
	}
...
//	*ray = Ray(Pdisk + worldRadius * sundir,
//			-UniformSampleCone(u2, u3, cosThetaMax, x, y, sundir));
...
Spectrum InfiniteLight::Le(const Vector &dir) const {
	const UV uv(1.f - SphericalPhi(dir) * INV_TWOPI + shiftU,
			SphericalTheta(dir) * INV_PI + shiftV);
...
InfiniteLightPortal::InfiniteLightPortal(TexMapInstance *tx,
		const std::string &portalFileName) :
	InfiniteLight(tx) {
	// Read portals
...
Spectrum InfiniteLightPortal::Sample_L(const Scene *scene, const Point &p,
	...
	unsigned int portalIndex = Min<unsigned int> (Floor2UInt(portalCount * u2),
			portalCount - 1);
	...
	for (unsigned int i = 0; i < portalCount; ++i) {
		...
		if ((sampleNdotMinusWi > 0.f) && (N && Dot(*N, wi) > 0.f)) {
			...
			*pdf = distanceSquared / (sampleNdotMinusWi
					* portalAreas[portalIndex] * portalCount);
...
Spectrum InfiniteLightPortal::Sample_L(const Scene *scene, const float u0,
	...
	// Select one of the portals
	const unsigned int portalCount = portals->GetTotalTriangleCount();
	unsigned int portalIndex = Min<unsigned int> (Floor2UInt(portalCount * u4),
			portalCount - 1);
...
TriangleLight::TriangleLight(const AreaLightMaterial *mat,
		const unsigned int mshIndex, const unsigned int triangleIndex,
		const std::vector<ExtMesh *> &objs) {
	lightMaterial = mat;
	meshIndex = mshIndex;
