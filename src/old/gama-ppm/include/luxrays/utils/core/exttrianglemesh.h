...
public:
	...
	virtual Normal GetNormal(const unsigned int triIndex,
			const unsigned int vertIndex) const = 0;
	...
	virtual Normal InterpolateTriNormal(const unsigned int index,
			const float b1, const float b2) const = 0;
	virtual Spectrum InterpolateTriColor(const unsigned int index,
			const float b0, const float b1, const float b2) const = 0;
	virtual Spectrum InterpolateTriColor(const unsigned int index,
			const float b1, const float b2) const = 0;
	virtual UV InterpolateTriUV(const unsigned int index, const float b1,
			const float b2) const = 0;

	virtual void Sample(const unsigned int index, const float u0,
			const float u1, Point *p, float *b0, float *b1, float *b2) const = 0;
...
public:
	...
	ExtTriangleMesh(const unsigned int meshVertCount,
			const unsigned int meshTriCount, Point *meshVertices,
			Triangle *meshTris, Normal *meshNormals = NULL,
			Spectrum *meshColors = NULL, UV *meshUV = NULL) {
	...
	Normal InterpolateTriNormal(const unsigned int index, const float b1,
			const float b2) const {
		const Triangle &tri = tris[index];
		const float b0 = 1.f - b1 - b2;
		Normal N;// = b0 * normals[tri.v[0]] + b1 * normals[tri.v[1]] + b2 * normals[tri.v[2]];
	...
	Spectrum InterpolateTriColor(const unsigned int index, const float b0,
			const float b1, const float b2) const {
		const Triangle &tri = tris[index];
		return b0 * colors[tri.v[0]] + b1 * colors[tri.v[1]] + b2
				* colors[tri.v[2]];
	}

	Spectrum InterpolateTriColor(const unsigned int index, const float b1,
			const float b2) const {
		const Triangle &tri = tris[index];
		const float b0 = 1.f - b1 - b2;
		return b0 * colors[tri.v[0]] + b1 * colors[tri.v[1]] + b2
	...
	UV InterpolateTriUV(const unsigned int index, const float b1,
			const float b2) const {
		const Triangle &tri = tris[index];
		const float b0 = 1.f - b1 - b2;
...
public:
	...
	float GetTriangleArea(const unsigned int triIndex) const {
		...
		return Triangle::Area(GetVertex(tri.v[0]), GetVertex(tri.v[1]),
				GetVertex(tri.v[2]));
	}
	unsigned int GetTotalVertexCount() const {
	...
	unsigned int GetTotalTriangleCount() const {
		return mesh->GetTotalTriangleCount();
	}
