...
struct BBox {
  ...
  __HYBRID__ BBox(const Point& p1, const Point& p2) {
    pmin = Point(ppm::math::min(p1.x, p2.x), ppm::math::min(p1.y, p2.y), ppm::math::min(p1.z, p2.z));
    pmax = Point(ppm::math::max(p1.x, p2.x), ppm::math::max(p1.y, p2.y), ppm::math::max(p1.z, p2.z));
  }
