...
void Film::AddImageToBuffer(SampleFrameBuffer* sampleFrameBuffer) {
  ...
  std::lock_guard<std::mutex> lock(imageBufferMutex);
...
void Film::UpdateScreenBuffer() {
...
#if defined USE_PPMPA || defined USE_SPPMPA
  const float weight = imageBufferCount;
#else
  ...
  for (unsigned int i = 0; i < pixelCount; ++i) {
    ...
    if (weight > 0.f) {
      ...
      p[i].r = imageBuffer[i].r * invWeight;
      p[i].g = imageBuffer[i].g * invWeight;
      p[i].b = imageBuffer[i].b * invWeight;
    }
  }
...
//void Film::UpdateScreenBuffer() {
//
//  std::lock_guard<std::mutex> lock(sampleFrameBufferMutex);
//
//  switch (toneMapParams->GetType()) {
//    case TONEMAP_LINEAR: {
//      const LinearToneMapParams &tm = (LinearToneMapParams &) *toneMapParams;
