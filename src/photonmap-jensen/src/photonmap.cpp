...
/* irradiance_estimate computes an irradiance irradiance_estimate
 ...
 * photon map given the parameter in np
*/
//*********************************
...
//*********************************
    ...
    if (dist2 < np->dist2[0]) {
        ...
        if (np->found < np->max) {
            ...
            // delete largest element, insert new, and reorder th heap
...
//*********************************
    if (stored_photons > 1) {
        // allocate two temprary arrays for the balancing procedure
...
void Photon_map :: median_split(
		Photon **p,
		const int start,	// start of the photon block in array
...
// See "Realistic Image Syntesis using Photon Mapping" Chapter 6
