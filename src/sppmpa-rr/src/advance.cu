  ...
  const int ix = abs(int(hh.x));
      ...
      if (length > 0) {
        ...
        for (uint i = localList; i < localList + length; i++) {
          ...
          HitPointPositionInfo *ihp = &worker->workerHitPointsInfoBuff[hashBuff->hashGridListsBuff[i]];
      HitPointRadianceFlux *hp = &worker->workerHitPointsBuff[hashBuff->hashGridListsBuff[i]];
 ...
 */
__global__ void fullAdvanceRR(CUDA_Worker* worker, PPM* engine, PointerFreeScene *ss,
    PointerFreeHashGrid* hashBuff, PhotonPath *livePhotonPaths,
    unsigned long long* photonCount, uint photonTarget) {
  // Select the ray to check
...
unsigned long long AdvancePhotonPath_wrapper(CUDA_Worker* worker, PPM* engine, uint photonTarget) {
  ...
  cudaMemset(worker->photonCountBuff, 0, sizeof(unsigned long long));
  cudaMemcpy(worker->engineBuff, engine, sizeof(PPM), cudaMemcpyHostToDevice);
  cudaMemcpy(worker->workerBuff, worker, sizeof(CUDA_Worker), cudaMemcpyHostToDevice);
  cudaMemcpy(worker->ssBuff, engine->ss, sizeof(PointerFreeScene), cudaMemcpyHostToDevice);
  cudaMemcpy(worker->hashBuff, worker->lookupA, sizeof(PointerFreeHashGrid),
      cudaMemcpyHostToDevice);
  cudaMemset(worker->livePhotonPathsBuff, 0, worker->WORK_BUCKET_SIZE * sizeof(PhotonPath));
  ...
  (worker->workerBuff, worker->engineBuff, worker->ssBuff,worker->hashBuff,
      worker->livePhotonPathsBuff, worker->photonCountBuff, photonTarget);

  checkCUDAError("");

  cudaMemcpy(&photonCount, worker->photonCountBuff, sizeof(unsigned long long),
  ...
  float MPhotonsSec = photonCount / ((WallClockTime()-start) * 1000000.f);

  printf("\nGPU Rate: %.3f MPhotons/sec\n",MPhotonsSec);

  return photonCount;

}

__global__ void AccumulateFluxPPMPA(CUDA_Worker* worker, PPM* engine, u_int64_t photonTraced) {

  int tid = blockIdx.x * blockDim.x + threadIdx.x;

  if (tid < engine->hitPointTotal) {

    HitPointPositionInfo *ehp = &worker->workerHitPointsInfoBuff[tid];
...
case    CONSTANT_COLOR:
    ...
    if ((ihp->accumPhotonCount > 0)) {
      ...
      //out of the loop
      const double k = 1.0 / (M_PI * worker->currentPhotonRadius2 * photonTraced);
...
void AccumulateFluxPPMPA_wrapper(CUDA_Worker* worker, uint photonTraced) {
  ...
  dim3 blockDIM = dim3(BLOCKSIZE, 1);
  dim3 gridDIM = dim3(ceil(engine->hitPointTotal / BLOCKSIZE));

  AccumulateFluxPPMPA<<<gridDIM,blockDIM>>>(worker->workerBuff, worker->engineBuff,photonTraced);
