...
QBVHAccel::QBVHAccel(u_int mp, u_int fst, u_int sf) :
...
void QBVHAccel::CreateSwizzledLeaf(int32_t parentIndex, int32_t childIndex,
  ...
  for (u_int q = 0; q < nbQuads; ++q) {
    new (&prims[primNum]) QuadTriangle(triangles, vertices,
        primsIndexes[primOffset], primsIndexes[primOffset + 1],
        primsIndexes[primOffset + 2], primsIndexes[primOffset + 3]);
...
void* QBVHAccel::GetNodes() const{
  return (void*) nodes;
}

uint QBVHAccel::GetNodesCount() const{
  return nNodes;
}

void* QBVHAccel::GetPrims() const{
  return (void*) prims;
}

uint QBVHAccel::GetPrimsCount() const{
  return nQuads;
}
