...
public:
	...
	void CamTranslate(const Vector &t) {
		if (motionBlur) {
			mbOrig = orig;
			mbTarget = target;
		}
	...
	void CamRotate(const float angle, const Vector &axis) {
		if (motionBlur) {
			mbOrig = orig;
			mbTarget = target;
		}
		...
		Transform t = TRotate(angle, axis);
		target = orig + t(p);
	}
	...
	void Update(const unsigned int filmWidth, const unsigned int filmHeight) {
		...
		Transform ScreenToRaster =
				...
				TTranslate(Vector(-screen[0], -screen[3], 0.f));
