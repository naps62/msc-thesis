...
public:
	...
	virtual Spectrum GetSahdowTransparency() const {
		throw std::runtime_error(
				"Internal error, called Material::GetSahdowTransparency()");
	}
};
...
public:
	...
	virtual Spectrum Le(const ExtMesh *mesh, const unsigned triIndex,
			const Vector &wo) const = 0;
};

class AreaLightMaterial: public LightMaterial {
public:
	AreaLightMaterial(const Spectrum &col) {
		gain = col;
	}
...
public:
	...
	virtual Spectrum
			f(const Vector &wo, const Vector &wi, const Normal &N) const = 0;
...
public:
	...
	Spectrum Sample_f(const Vector &/*wo*/, Vector *wi, const Normal &/*N*/,
		...
		*pdf = dir.z * INV_PI;
		...
		dir = Vector(v1.x * dir.x + v2.x * dir.y + shadeN.x * dir.z,
...
public:
	...
	bool HasSpecularBounceEnabled() const {
		return reflectionSpecularBounce;
	}
...
public:
	MatteMirrorMaterial(const Spectrum &col, const Spectrum refl,
	...
	Spectrum Sample_f(const Vector &wo, Vector *wi, const Normal &N,
		...
		if (onlySpecular || (comp > matteFilter)) {
			const Spectrum f = mirror.Sample_f(wo, wi, N, shadeN, u0, u1, u2,
					onlySpecular, pdf, specularBounce);
			*pdf *= mirrorPdf;

			return f;
		} else {
			const Spectrum f = matte.Sample_f(wo, wi, N, shadeN, u0, u1, u2,
					onlySpecular, pdf, specularBounce);
			*pdf *= mattePdf;
	...
	const MatteMaterial &GetMatte() const {
		return matte;
	}
	const MirrorMaterial &GetMirror() const {
		return mirror;
	}
	float GetMatteFilter() const {
		return matteFilter;
	}
	float GetTotFilter() const {
		return totFilter;
	}
	float GetMattePdf() const {
		return mattePdf;
	}
	float GetMirrorPdf() const {
		return mirrorPdf;
	}
...
public:
	...
	bool HasRefrctSpecularBounceEnabled() const {
		return transmitionSpecularBounce;
	}
...
public:
	MatteMetalMaterial(const Spectrum &col, const Spectrum refl,
			const float exp, bool reflSpecularBounce) :
	...
	Spectrum Sample_f(const Vector &wo, Vector *wi, const Normal &N,
		...
		if (onlySpecular || (comp > matteFilter)) {
			 const Spectrum f = metal.Sample_f(wo, wi, N, shadeN, u0, u1, u2,
					onlySpecular, pdf, specularBounce);
			*pdf *= metalPdf;
...
//					}
	...
	const MetalMaterial &GetMetal() const {
		return metal;
	}
	float GetMatteFilter() const {
		return matteFilter;
...
public:
	AlloyMaterial(const Spectrum &col, const Spectrum &refl, const float exp,
			const float schlickTerm, bool reflSpecularBounce) {
		Krefl = refl;
		Kdiff = col;
	...
	Spectrum Sample_f(const Vector &wo, Vector *wi, const Normal &/*N*/,
		...
		if (onlySpecular || (u2 < P)) {
			(*wi) = MetalMaterial::GlossyReflection(wo, exponent, shadeN, u0,
					u1);
			*pdf = P / Re;
			specularBounce = reflectionSpecularBounce;
