...
void CUDA_Worker::getDeviceHitpoints() {
	...
	cudaMemcpy(hitPoints_iterationCopy, workerHitPointsBuff, sizeof(HitPoint) * engine->hitPointTotal,
			cudaMemcpyDeviceToHost);
...
void CUDA_Worker::updateDeviceHitPoints() {
	...
	if (!workerHitPointsInfoBuff) {
		cudaMalloc((void**) (&workerHitPointsInfoBuff),
				sizeof(HitPointStaticInfo) * engine->hitPointTotal);

#if defined USE_SPPMPA || defined USE_SPPM
	}
#endif

	cudaMemcpy(workerHitPointsInfoBuff, &hitPointsStaticInfo_iterationCopy[0],
			sizeof(HitPointStaticInfo) * engine->hitPointTotal, cudaMemcpyHostToDevice);
	...
	if (!workerHitPointsBuff)
		cudaMalloc((void**) (&workerHitPointsBuff), sizeof(HitPoint) * engine->hitPointTotal);

	cudaMemcpy(workerHitPointsBuff, &hitPoints_iterationCopy[0], sizeof(HitPoint) * engine->hitPointTotal,
			cudaMemcpyHostToDevice);
