...
public:
	...
	__HYBRID__ kernel(gama::vector<int> _arr, unsigned int _size, unsigned int lo, unsigned int hi)
	: arr(_arr), size(_size), lower(lo), upper(hi)
	{
	...
	__DEVICE__ List<work*>* dice(unsigned int &number) {
		...
		for(unsigned k = 0; k < number; ++k) {
			...
			*s = kernel(arr, size, start, start + number_of_units);
			(*L)[k] = s;
