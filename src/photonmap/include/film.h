...
//#endif
	...
	__HD__
	void SplatRadiance(SampleFrameBuffer* sampleFrameBuffer,
			const Spectrum radiance, const unsigned int x, const unsigned int y,
...
//	}
	__HD__
	void SplatFiltered(SampleFrameBuffer* sampleFrameBuffer,
			const SampleBufferElem *sampleElem) {
		...
		for (int iy = y0; iy < y1; ++iy) {
			...
			for (int ix = x0; ix < x1; ++ix) {
				...
				SplatRadiance(sampleFrameBuffer, sampleElem->radiance, ix, iy,
						filterWt);
			}
		}
	}
	...
	/*
	 * oversample reduction happens here
	 */
	void SplatSampleBuffer(SampleFrameBuffer* sampleFrameBuffer,
		...
		for (unsigned int i = 0; i < cfg->hitPointTotal; ++i) {
			...
			/**
			 * In PPM stratified sampling moves the sample potentially of
			 * the pixel. Casting puts it in other pixel radiance. In SPPM
			 * there always one sample and it assures that it contributes to the
			 * proper pixel.
			 */
			if (cfg->GetEngineType() == SPPM
					|| cfg->GetEngineType() == SPPMPA) {

				scrX = (uint) (sbe[i].id % cfg->width);
				scrY = (uint) (sbe[i].id / cfg->width);

			} else {
				scrX = (uint) sbe[i].screenX;
				scrY = (uint) sbe[i].screenY;

			}

			SplatRadiance(sampleFrameBuffer, sbe[i].radiance, scrX, scrY);
