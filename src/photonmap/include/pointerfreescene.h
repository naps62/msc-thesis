...
public:
	...
	void SkyLight_GetSkySpectralRadiance(const float theta, const float phi,
		...
		// Compute xyY values
		const float x = skyLight->zenith_x
				* SkyLight_PerezBase(skyLight->perez_x, theta_fin, gamma);
		const float y = skyLight->zenith_y
				* SkyLight_PerezBase(skyLight->perez_y, theta_fin, gamma);
		const float Y = skyLight->zenith_Y
				* SkyLight_PerezBase(skyLight->perez_Y, theta_fin, gamma);
	...
	void GenerateRay(const float screenX, const float screenY,
		...
		const float iw = 1.f
				/ (camera->rasterToCameraMatrix[3][0] * Pras.x
		...
		Point torig;
		const float iw2 = 1.f
				/ (camera->cameraToWorldMatrix[3][0] * orig.x
	...
	__HD__
	void SampleTriangleLight(POINTERFREESCENE::TriangleLight *light,
			const float u0, const float u1, Point *p) {
		Point v0, v1, v2;
		v0 = light->v0;
		v1 = light->v1;
		v2 = light->v2;
	...
	void TriangleLight_Sample_L(POINTERFREESCENE::TriangleLight *l,
			...
			const float u4, float *pdf, Ray *ray, Spectrum& f, Spectrum* colors,
	...
	void Mesh_InterpolateColor(Spectrum *colors, Triangle *triangles,
		...
		C->r = b0 * colors[tri->v[0]].r + b1 * colors[tri->v[2]].r
				+ b2 * colors[tri->v[2]].r;
		C->g = b0 * colors[tri->v[0]].g + b1 * colors[tri->v[1]].g
				+ b2 * colors[tri->v[2]].g;
		C->b = b0 * colors[tri->v[0]].b + b1 * colors[tri->v[1]].b
				+ b2 * colors[tri->v[2]].b;
	}
	__HD__
	...
	void Mesh_InterpolateUV(UV *uvs, Triangle *triangles, const uint triIndex,
		...
		const float b0 = 1.f - b1 - b2;
		uv->u = b0 * uvs[tri->v[0]].u + b1 * uvs[tri->v[1]].u
				+ b2 * uvs[tri->v[2]].u;
		uv->v = b0 * uvs[tri->v[0]].v + b1 * uvs[tri->v[1]].v
				+ b2 * uvs[tri->v[2]].v;
	}
	__HD__
	...
	void inline SunLight_Sample_L(Point *hitPoint, float *pdf, Spectrum *f,
			Ray *shadowRay, float u0, float u1,
			POINTERFREESCENE::SunLight *sunLight) {
		const float cosThetaMax = sunLight->cosThetaMax;
	...
	void inline SunLight_Sample_L(const float u0, const float u1,
		...
		Point Pdisk = worldCenter
				+ worldRadius * (d1 * sunLight->x + d2 * sunLight->y);

		// Set ray origin and direction for infinite light ray
		*ray = Ray(Pdisk + worldRadius * sunLight->sundir,
				-UniformSampleCone(u2, u3, sunLight->cosThetaMax, sunLight->x,
						sunLight->y, sunLight->sundir));
		*pdf = UniformConePdf(sunLight->cosThetaMax)
	...
	__HD__
	void inline MatteMetal_f(POINTERFREESCENE::MatteMetalParam *mat,
			const Vector &wo, const Vector &wi, const Normal &N, Spectrum& f) {
		Matte_f(&mat->matte, wo, wi, N, f);
		f *= mat->mattePdf;
	}
	...
	__HD__
	void inline Mirror_Sample_f(POINTERFREESCENE::MirrorParam *mat,
			const Vector *wo, Vector *wi, float *pdf, Spectrum *f,
			const Normal *shadeN, bool *specularBounce) {
	...
	__HD__
	void inline Glass_Sample_f(POINTERFREESCENE::GlassParam *mat,
			const Vector *wo, Vector *wi, float *pdf, Spectrum *f,
	...
	__HD__
	void inline Metal_Sample_f(POINTERFREESCENE::MetalParam *mat,
			const Vector *wo, Vector *wi, float *pdf, Spectrum *f,
			const Normal *shadeN, const float u0, const float u1,
			bool *specularBounce) {
	...
	__HD__
	void inline Alloy_Sample_f(POINTERFREESCENE::AlloyParam *mat,
			const Vector *wo, Vector *wi, float *pdf, Spectrum *f,
			const Normal *shadeN, const float u0, const float u1,
			const float u2, bool *specularBounce) {
		// Schilick's approximation
