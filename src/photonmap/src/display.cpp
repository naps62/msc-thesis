...
static void PrintCaptions() {
	...
	glRecti(0, cfg->height - 15, cfg->width - 1, cfg->height - 1);
	glRecti(0, 0, cfg->width - 1, 18);
	glDisable(GL_BLEND);

	// Title
	glColor3f(1.f, 1.f, 1.f);
	glRasterPos2i(4, cfg->height - 10);
	PrintString(GLUT_BITMAP_8_BY_13, cfg->SPPMG_LABEL);

	// Stats
	glRasterPos2i(4, 5);
	char captionBuffer[512];
	const double elapsedTime = WallClockTime() - cfg->startTime;
	const unsigned int kPhotonsSec = engine->getPhotonTracedTotal()
			/ (elapsedTime * 1000.f);
	sprintf(captionBuffer,
			"[It %d][Photons %.2fM][Avg. photons/sec % 4dK][Elapsed time %dsecs]",
			engine->GetIterationNumber(),
			float(engine->getPhotonTracedTotal() / 1000000.0), kPhotonsSec,
			int(elapsedTime));
...
static void DisplayFunc(void) {
	...
	if (engine->filmCreated()) {
		...
		glRasterPos2i(0, 0);
		glDrawPixels(cfg->width, cfg->height, GL_RGB, GL_FLOAT,
...
static void TimerFunc(int value) {
	...
	glutTimerFunc(cfg->scrRefreshInterval, TimerFunc, 0);
}
...
void InitGlut(int argc, char *argv[], const unsigned int width,
	...
	if ((scrWidth + 50 < width) || (scrHeight + 50 < height))
		...
		glutInitWindowPosition((scrWidth - width) / 2,
				(scrHeight - height) / 2);

	glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE);
	glutCreateWindow(cfg->SPPMG_LABEL);
}
