...
void CUDA_Worker::ProcessEyePaths() {
	...
	GenenerateCameraRays_wrapper(this);

	double start = WallClockTime();

	unsigned long long r = BuildHitpoints_wrapper(this);
...
void CUDA_Worker::UpdateBBox() {
	...
	//double elapse = WallClockTime() - start;
...
u_int64_t CUDA_Worker::BuildPhotonMap(u_int64_t iterationPhotonTarget) {
	...
	uint slices = IntDivUp(iterationPhotonTarget, PHOTONS_PER_SLICE);

	uint photonTarget = PHOTONS_PER_SLICE;

	for (int i = 0; i < slices; i++) {

		if (i == (slices - 1))
			photonTarget = iterationPhotonTarget - (i * PHOTONS_PER_SLICE);
	...
	float MPhotonsSec = iterationPhotonTarget / ((WallClockTime() - start) * 1000000.f);
...
void CUDA_Worker::updateDeviceHitPointsInfo(bool toHost) {
	...
	if (toHost)
		cudaMemcpy(cfg->GetEngine()->GetHitPoints(), workerHitPointsInfoBuff,
				sizeof(HitPoint) * cfg->hitPointTotal, cudaMemcpyDeviceToHost);
	else
		cudaMemcpy(workerHitPointsInfoBuff, cfg->GetEngine()->GetHitPoints(),
				sizeof(HitPoint) * cfg->hitPointTotal, cudaMemcpyHostToDevice);
...
void CUDA_Worker::CopyAcc() {
	...
	cudaMalloc((void**) &d_qbvhBuff,
			sizeof(POINTERFREESCENE::QBVHNode) * nNodes);
	cudaMalloc((void**) &d_qbvhTrisBuff,
...
void CUDA_Worker::AllocCopyCUDABuffer(void **buff, void *src, const size_t size,
		const string &desc) {
...
void CUDA_Worker::GetSampleBuffer() {
	...
	cudaMemcpy(sampleBuffer->samples, sampleBufferBuff,
			sizeof(SampleBufferElem) * cfg->hitPointTotal,
			cudaMemcpyDeviceToHost);
