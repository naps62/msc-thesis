...
Frameworks targeting \hetplats are still an emerging solution for parallel computing. \gama and \starpu are promising proposals, with the latter providing a solid API, backed by a relatively large user-base but with still some problems to be solved. Design issues in the API allow common an easy to make developer errors to cause unexpected behaviour.
