...
%\cref{fig:starpu_deps} shows some of the possibilities of this model. Tasks $A$ and $B$ have read-only access to the data buffer $x$, while task C has write-only access.
...
\item[dm (Deque Model)] Performs a HEFT-like strategy, similarly to \gama (see \cref{sec:gama_sched}). Each task is assigned to where the scheduler thinks its termination time will be minimized; \todo{LPS: explicar o heft, com referencias}
