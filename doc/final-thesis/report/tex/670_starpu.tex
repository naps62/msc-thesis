...
The fact that the number of concurrent iterations is now configurable also allowed to tackle the data dependency provided by the seed buffer. Instead of a single buffer being shared by the entire algorithm, the implementation was adapted to use as many buffers as there are concurrent iterations, as shown in \todo{diagrama de iterações concorrentes}.
