...
Large efforts have been made recently to improve ray tracing techniques \todo{refs aqui}, as well as to speed up their computation, by using parallel architectures such as \gpus, which have always been associated with image processing.
\todo{LPS: isto já não é bem assim. ver por exemplo o optiX da NVidia}
...
\todo{LPS: note that these rendering algorithms also use ray casting as the visibility evaluation algorithm (although radiosity often resorts to hemicube projection if run on the GPU)}
